 ...
 * 教學語言採 Anthropic「AI Fluency」的 4D 能力（委派 Delegation / 描述 Description /
 * 辨別 Discernment / 審慎 Diligence），並與裁判的五維度評分對齊。
 */
export async function runExemplar(input: ExemplarInput): Promise<string> {
  const { text } = await generateText({
    ...
    system: [
      ...
      '',
      '教學用 Anthropic「AI Fluency」的 4D 能力當共同語言，四項都要在文中出現、並各自對應具體做法：',
      '- 委派 Delegation：判斷哪些子任務適合交給 AI、如何框定問題邊界、如何分階段推進而非一次全丟。',
      '- 描述 Description：給足角色、背景脈絡、輸出格式與字數、驗收標準；過程中對 AI 給明確回饋。',
      '- 辨別 Discernment：不預設 AI 說的對，主動檢查它的作法與產出，能看出漏洞與可疑數字。',
      '- 審慎 Diligence：採用任何關鍵數字 / 事實前，自己再獨立核一遍；逐項對照限制條件；為最終成品負責。',
      '',
      '結構固定為三段（用 `##` 標題）；每段開頭用一句話點出它主要練的是哪一個 D：',
      '',
      '## 理想的開場提示詞（描述 Description，也體現委派 Delegation 的任務框定）',
      '給一段可以直接複製使用的提示詞範例，放在 ```` ``` ```` 圍起的 code block 裡。',
      '要展示任務說明「沒寫、但高手會自己補上」的東西：角色設定、背景脈絡、輸出格式與字數、驗收標準。',
      '',
      '## 協作方式（委派 Delegation ＋ 辨別 Discernment）',
      '3–5 個要點：怎麼把任務拆成可驗證的小步、分階段收斂到成品、怎麼要求 AI 說明推理過程並檢查它的關鍵數字/事實。',
      '',
      input.trapEffective
        ? '## 這題的陷阱（辨別 Discernment ＋ 審慎 Diligence）\n對話中 AI 會講一項聽起來合理但錯誤的資訊。說明高手會怎麼察覺（例如自己心算/列式獨立驗一次），' +
          '並提醒：光叫 AI「自己檢查」通常沒用，它會堅持原答案——要嘛請它逐步列出算式一步步核對、要嘛自己算完直接告訴它正確值。最後寫出正確版本。'
        : '## 最容易踩的坑（辨別 Discernment ＋ 審慎 Diligence）\n這類任務最容易被 AI 誤導、或自己忽略的一點，以及採用前該怎麼獨立查證。',
      '',
