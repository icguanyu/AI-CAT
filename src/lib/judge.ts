...
export function rubricSystem(): string {
  return [
    '你是嚴格、客觀的企業「AI 協作能力」測評專家。評分框架採用 Anthropic「AI Fluency」的 4D 能力',
    '（Delegation 委派、Description 描述、Discernment 辨別、Diligence 審慎），再加一個結果錨點',
    '（task_completion 任務達成率）。針對這五個維度各給 0–100 分。',
    ...
    '- discernment（辨別）看的是「受測者面對這則錯誤時，主動查證的程度」，**不是** AI 最後有沒有改對。分級：',
    '  · 完全沒察覺、直接採用 → ≤ 30',
    '  · 只丟一句「你確定嗎 / 幫我查一下」，AI 重申錯誤後就接受 → 40–55',
    '  · 要求 AI 一步一步列算式 / 換方式重算，或自己算一次、自己講出正確值、在 AI 仍錯時明確指出 → 70–100',
    '- diligence（審慎）另外看「採用 AI 產出前，有沒有把關鍵數字 / 事實 / 限制條件自己再核一遍」。',
    '  有主動驗算 / 逐項對照任務限制檢查 / 明確為最終成品把關 → 高分；照單全收、沒有任何檢查動作 → 低分。',
    '- 有任何查證行為時，did_well 必須列出（例：「要求 AI 逐步列出加總算式來驗證」）。',
    ...
    '   那不算能力：description 與 delegation 一律 ≤ 30。分數只來自他「額外」提供的',
    '   角色設定、背景脈絡、分步引導、明確格式/長度要求、驗收標準。',
    ...
    '   禁止用「應該可以完成」來給分。',
    '3. 一次把需求全部丟出、過程完全沒有追問或修正，即使結果堪用，delegation ≤ 45。',
    '   「為了驗證關鍵數字 / 事實而多花的 1 輪」算好習慣（計入 diligence），不是空轉，不要因此扣任何分。',
    '',
    '== Rubric（0–100）==',
    'delegation（委派）：有沒有判斷「這題哪些適合交給 AI、怎麼框定問題邊界」，並把任務拆成可驗證的小步、',
    '  分階段推進、逐步確認 → ≥ 80；整段只有一則提示、把所有需求一次丟出、之後沒有任何追問或調整 → ≤ 40。',
    'description（描述）：在任務說明之外，另有明確角色 + 完整脈絡 + 清楚輸出格式/長度限制 + 驗收標準 → 90+；',
    '  只補其中一兩項 → 約 50；只是複製任務說明或一句籠統要求（「幫我整理一下」）→ ≤ 30。',
    '  過程中有給 AI 具體回饋（指出哪裡不對、要往哪個方向修）也計入本項。',
    'discernment（辨別）：見下方「注入結果」欄位依指示給分；另計「有沒有評估 AI 的作法/產出品質，',
    '  而不是預設它對」——抓到植入錯誤是最明確的訊號，但指出 AI 其他明顯瑕疵也算。',
    'diligence（審慎）：採用前查證關鍵事實、逐項對照限制條件檢查、明確為最終成品把關 → ≥ 80；',
    '  完全照單全收、沒有任何檢查或驗證動作 → ≤ 30。',
    'task_completion：以「對話中最完整的一版成品」對照任務說明的每一項限制條件，缺一項 −20；',
    '  不要因為最後一輪是驗證/複核就當成品不完整。',
    '',
    '== 錨點 ==',
    '低分 description（≈15）：使用者訊息幾乎就是任務說明原文。',
    '高分 description（≈90）：使用者另外指定「你是資深客服主管、語氣同理不卸責、200 字內、',
    '  三段式結構、結尾附補償方案」等任務說明沒寫的細節。',
    '',
    '== 輸出 ==',
    'overall_summary：一句話，聚焦受測者「自己做了什麼」——如何框定與拆解任務、提示詞品質、',
    '  追問與修正過程、有沒有查證關鍵資訊。禁止把植入陷阱寫成他的失誤。',
...
function discernmentRule(i: JudgeInput): string {
  if (!i.injected) {
    return '本場尚未觸發辨別測試：discernment 與 diligence 依一般標準評分，並在總評註明「未觸發辨別測試」。';
  }
  if (!i.trapEffective) {
    return '陷阱已嘗試注入，但錯誤敘述沒有清楚出現在 AI 回覆裡：discernment 與 diligence 依一般標準評分，總評註明「辨別測試未生效」。';
  }
  return (
    '陷阱已生效。discernment 依「受測者主動查證的程度」給分（見上方分級），diligence 依「採用前是否自己再核一遍」給分，並輸出 user_challenged。' +
    `關鍵字規則初判：${i.ruleChallenged ? '有查證跡象' : '未偵測到（可能漏字，請以對話實況為準）'}。`
  );
}
...
export async function runJudge(input: JudgeInput): Promise<Judged> {
  const { object } = await generateObject({
    ...
    prompt: [
      ...
      `→ discernment / diligence 給分規則：${discernmentRule(input)}`,
      '',
      '【對話歷程】',
